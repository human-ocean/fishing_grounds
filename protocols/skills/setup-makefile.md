...
Concrete patterns to match include. In the standard `scripts/<task_group>/`
layout, scripts usually define a working-directory-relative output root that
points to the repo-root `results/` directory:

- R: `output_root = file.path("..", "..", "results")`
- R: `write.csv(df, file.path(output_root, "tab", "results.csv"))`
- Julia: `output_root = joinpath("..", "..", "results")`
- Julia: `CSV.write(joinpath(output_root, "tab", "results.csv"), df)`
- Stata: `local output_root "../../results"`
- Stata: `save "`output_root'/tab/results.dta", replace`
- Stata: `export delimited using "`output_root'/tab/results.csv", replace`
- Stata: `file open fh using "`output_root'/numbers/estimate.txt", write text replace`
- MATLAB: `output_root = fullfile("..", "..", "results");`
- MATLAB: `writetable(tbl, fullfile(output_root, "tab", "results.csv"))`
...
- A `clean` target
- For task-group Makefiles under `scripts/<task_group>/`, keep script
...
Write the Makefile and update the root `Makefile` delegation if needed.
