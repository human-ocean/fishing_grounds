...
files containing `\newcommand` definitions in `results/numbers/`. The
Makefile exposes those files through `TEXINPUTS`.
...
1. Glob for `results/numbers/*.txt`.
...
- For `latex/main.tex` and other manuscript-like `.tex` files, review
...
1. Search `scripts/` for the numeric value and contextual keywords.
...
If `results/numbers/` does not exist and `scripts/` has no scripts, report that
