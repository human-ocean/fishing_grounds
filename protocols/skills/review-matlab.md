...
1. **Identify scripts to review:**
   ...
   - If the argument is `all`, review all MATLAB scripts in `scripts/`, excluding
     `scripts/unit_tests/` and `scripts/hpc/`.

2. **For each script, follow the review protocol below:**
   - Read the MATLAB section of `scripts/AGENTS.md`.
...
- Paths relative to the script working directory (typically `scripts/<task_group>/`)
