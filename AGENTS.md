...
<!-- HOW TO USE: Replace [BRACKETED PLACEHOLDERS] with your project info.
     ...
     Code-specific conventions are in scripts/AGENTS.md.
...
| Utilities | `R/` | Complete | Reusable functions |
...
Plan approved -> orchestrator activates
  ...
  |         If Makefile exists: `make [target]`
...
| Major | results/numbers/ file missing from Makefile dependencies | -5 |
...
|----------|-----------|-----------|
| Point estimates | 1e-6 | Numerical precision in fixest |
| Standard errors | 1e-4 | Acceptable rounding in Conley SE |
| Coverage rates | N/A | No simulation component |
...
2. Build stale targets: `make [target]` or `make -C latex`
...
6. Verify all dynamic number `\input{...}` files exist in `results/numbers/`

### For R Scripts:
1. Prefer `make [target]`; fall back to `Rscript path/to/script.R`
2. Verify output files (PDF, RDS, CSV) were created with non-zero size
3. Spot-check estimates for reasonable magnitude

### For Julia Scripts:
1. Prefer `make [target]`; fall back to `julia path/to/script.jl`
...
### For Stata Scripts:
1. Prefer `make [target]`; fall back to `stata -b do path/to/script.do`
...
### For MATLAB Scripts:
1. Prefer `make [target]`; fall back to `matlab -batch "run('path/to/script.m')"`
...
### For Code Pipelines:
1. When the task adds, renames, or restructures scripts under `scripts/`, verify
